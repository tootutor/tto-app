...
Additional modified by Witoon S. to support / and %
...
(AMfunc = function(doAMprocessor) {
	...
	{input:"//", tag:"mo", output:"/",      tex:null, ttype:DEFINITION}, //Changed from CONST by tto
...
// MUZ HACK: This next one gave problems in probability chapter	
	{input:"|", tag:"mo", output:"|", tex:null, ttype:LEFTRIGHT},
// This next one was commented out already...
	//{input:"||", tag:"mo", output:"||", tex:null, ttype:LEFTRIGHT},
		...
		if (len>2 && newFrag.charAt(0)=='{' && newFrag.indexOf(',')>0) { //could be matrix (total rewrite from .js)
			...
			if (right==')' || right==']') {
				...
				if ((left=='(' && right==')' && symbol.output != '}') || (left=='[' && right==']')) {
					...
					mxout += '\\end{matrix}}';
					// MUZ HACK: Remove endless { } brackets so matrices and determinants work
					
					var mxoutFrac = mxout.split("\\frac");
//console.log(mxoutFrac);					
					// Split on { } brackets
					var mxoutArr = mxout.split(/[{\}]{1,2}/);

//console.log(mxoutArr);
					var newmxout = mxoutArr[0] + "{"+mxoutArr[1] + "}";
					var muzRem=2, isFrac=0;
					for(var muz = 2; muz < mxoutArr.length-3; muz++) {
//console.log("muz="+muz+", muzRem="+muzRem);						
						if(mxoutArr[muz] == "\\frac" || mxoutArr[muz] == "-\\frac" || (isFrac==1 && muz == muzRem + 2)  ) {
							newmxout += mxoutArr[muz]+"{";
							isFrac=1;
							muzRem = muz;
//console.log("trigger! muz="+muz+", muzRem="+muzRem);
						} else if(isFrac==1) {
							if(muz == muzRem+1 || muz == muzRem+3) {
								newmxout += mxoutArr[muz] + "}";
							}
							if(muz == muzRem+3) {
								isFrac = 0;
					...
					newmxout += mxoutArr[mxoutArr.length-3] + "{" + mxoutArr[mxoutArr.length-2] + "}";
//console.log(newmxout);					
					if (matrix) { newFrag = newmxout;}
				}
			}
		}
	...
	function AMparseMath(str) {
	 //DLMOD to remove &nbsp;, which editor adds on multiple spaces
	  str = str.replace(/(&nbsp;|\u00a0|&#160;)/g,"");
		// MUZ HACKS
		// These did not work as texstring replacements; need to be str
		str = str.replace(/&gt;/g,">");
		str = str.replace(/&lt;/g,"<");
	   ...
	   str = str.replace(/’/g,"text(')");
		...
		// Laplace symbol (no joy)
		//str = str.replace(/ℒ/g,"ℒ");  //&#8466;
...
////////////////// MUZ HACKS /////////////////////////////////////////////
	  ...
	  texstring = texstring.replace(/∞/g,"\\infty");
		...
		node.id = "mathId"+counter;
		node.innerHTML = texstring;
		try {
			katex.render(texstring,node);
		} catch(err) { // So MathJax can handle the expressions that KaTeX cannot			
			if (typeof MathJax !== 'undefined') {
				node.innerHTML = "`"+str+"`";
				MathJax.Hub.Queue(["Typeset",MathJax.Hub,"mathId"+counter]);
	...
	if (typeof(doAMprocessor) != "undefined") {	
		 translate();
	};
})();